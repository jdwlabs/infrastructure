...
- `clusters/<name>/` — per-cluster runtime state. Plaintext working files (`secrets/`, `nodes/`, `state/`) are gitignored; the encrypted `vault/*.enc.yaml` is committed (SOPS+age — see `docs/secrets.md`)
